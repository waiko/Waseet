...
class MyCartFragment : BaseFragment() {
            ...
            override fun onItemClick(
                ...
                when (view.id) {
                    ...
                    R.id.ivIncreaseQuantity -> {
                        val qty = dataBinding.edtQty.text.toString()
                        if (model.stock_quantity != null) {
                            if (qty.toInt() < model.stock_quantity) {
                                dataBinding.edtQty.text = qty.toInt().plus(1).toString()
                                val requestModel = RequestModel(); requestModel.pro_id =
                                    model.pro_id; requestModel.quantity =
                                    qty.toInt().plus(1); requestModel.cart_id =
                                    model.cart_id.toInt()

                                updateCartItem(requestModel)
                            } else {
                                activity?.snackBarError("${getString(R.string.lbl_qty_error)} ${model.stock_quantity}")
                            }
                        } else {
                            activity?.snackBarError(getString(R.string.lbl_out_stock))
                        }
                    }
                    R.id.ivDecreaseQuantity -> {
                        ...
                        if (model.stock_quantity != null) {
                            if (qty.toInt() != 0) {
                                if (qty.toInt() < model.stock_quantity) {
                                    dataBinding.edtQty.text = qty.toInt().minus(1).toString()
                                    notifyItemChanged(position)
                                    val requestModel = RequestModel(); requestModel.pro_id =
                                        model.pro_id; requestModel.quantity =
                                        qty.toInt().minus(1); requestModel.cart_id =
                                        model.cart_id.toInt()
    ...
    private fun updateCartItem(model: RequestModel) {
        showProgress()
        callApi(getRestApis(false).updateItemInCart(request = model), onApiSuccess = {
